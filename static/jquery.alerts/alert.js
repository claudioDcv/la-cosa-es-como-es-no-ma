...
(function($) {
	...
	$.alerts = {
		...
		alert: function(message, ok, callback, timeout) {
		...
		_show: function(msg, value, ok, cancel, type, callback, timeout) {
			...
			switch( type ) {
				case 'alert':
					if (timeout) {
						$("#popup_message").after('<div id="popup_panel">' + ok + '</div>');
						setTimeout(function() {
							callback(true);	
						}, timeout);
					} else {
	...
	// Shortuct functions
	jAlert = function(message, ok, callback, timeout) {
		$.alerts.alert(message, ok, callback, timeout);
	}
